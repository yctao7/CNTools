...
def cns_info(ds, n_cns, cns, filename):
    data = ds.data
    ...
    logging.info(f'Entropy: {(info_ent * info_size / info_size.sum()).sum():.3f}')
    logging.info(f'Size: {sum(info_size) / sum(info_pat):.2f}')
...
def identify(ds_path, out_dir, cns_path, method, verbose, **params):
    ...
    if verbose:
        cns_info(ds, params['n_cns'], cns, filename)
    ...
    for tech in ['Naive', 'HMRF']:
        if params[tech]:
            ...
            if verbose:
                cns_info(ds, params['n_cns'], cns_smoothed, f'{filename}_{tech}')
...
required_CNE.add_argument('--eta', type=float, help='parameter to control Gaussian std')
