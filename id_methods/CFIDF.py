...
def CFIDF(ds, n_cns, eps, r, include_neighbors, n_included, seed):
    ...
    for sample in tqdm(data):
        for image in tqdm(data[sample], leave=False):
            ...
            for i in nb_cell[sample][image]:
                feats_nb[sample][image][i] = data[sample][image].cts_oh[nb_cell[sample][image][i]].mean(axis=0)
    feats_nb_normed = []
    for sample in data:
        for image in data[sample]:
            for nb_id in feats_nb[sample][image]:
                feats_nb[sample][image][nb_id] *= np.log(ds.n_cells / (ds.ct_counts + 1))
    ...
    for sample in data:
        for image in data[sample]:
            feats_normed_tmp = np.zeros(data[sample][image].cts_oh.shape)
